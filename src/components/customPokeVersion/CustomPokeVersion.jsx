...
export default function CustomPokeVersion({entrie, index, setIndexFlavor, color}){
  ...
  return(
    <div class='version' ref={thisVersion} >
      <div className={entrie.version.name} >
        <svg onClick={()=>{clickVersion(index)}} onMouseLeave={()=>{setIsHover(false)}} onMouseOver={()=>{ setDomData(thisVersion.current.getBoundingClientRect()); setIsHover(true)}} width='30' height='30' viewBox=' 0 0 30 30' xmlns='http://www.w3.org/2000/svg' xmlnsXlink='http://www.w3.org/1999/xlink'>
      ...
      {isHover ? <span style={{ left: position.x, top: position.y }} ref={textThisVersion} className='tooltip'> {entrie.version.name} </span> : ''}
    </div>
  )
}